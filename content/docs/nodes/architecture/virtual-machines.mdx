...
3. Only the active proposer can build during its slot; missed slots pass to the next sampled validator. Eligibility never opens beyond the validator set at the parent's embedded P-Chain height (the pre-Durango any-node fallback is gone); see [ProposerVM](/docs/nodes/architecture/proposervm).
