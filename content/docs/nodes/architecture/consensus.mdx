...
4. If a proposer misses its window, the schedule moves to the next sampled validator in 5-second slots. Only validators in the set at the parent block's embedded P-Chain height are ever eligible: the pre-Durango fallback that opened production to any node after the final window no longer exists.
...
- **Liveness within the set**: An offline proposer only delays the next scheduled validator. If no validator from that set is still live on the P-Chain, production halts; see [ProposerVM](/docs/nodes/architecture/proposervm).
