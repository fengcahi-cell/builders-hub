...
export async function getBlockchainInfo(
  blockchainId: string,
  signal?: AbortSignal,
): Promise<BlockchainInfo & { isTestnet: boolean }> {
...
export async function getBlockchainInfoForNetwork(
  network: Network,
  blockchainId: string,
  signal?: AbortSignal,
...
export async function getSubnetInfo(
  subnetId: string,
  signal?: AbortSignal,
): Promise<SubnetInfo & { isTestnet: boolean }> {
  // Get current network from wallet store
  const { avalancheNetworkID } = useWalletStore.getState();
  const currentNetwork = avalancheNetworkID === networkIDs.MainnetID ? 'mainnet' : 'testnet';
  const otherNetwork = currentNetwork === 'mainnet' ? 'testnet' : 'mainnet';
...
export async function getSubnetInfoForNetwork(
  network: Network,
  subnetId: string,
  signal?: AbortSignal,
): Promise<SubnetInfo> {
...
export async function getPChainBalance(network: Network, address: string): Promise<PChainBalanceResponse> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch P-Chain balance for ${address} on ${networkPath} (${network}): ${response.status} ${response.statusText}`,
    );
  }
...
export async function getNativeTokenBalance(chainId: string | number, address: string): Promise<NativeTokenBalance> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch native token balance for address ${address} on chain ${chainId}: ${response.status} ${response.statusText}`,
    );
  }
