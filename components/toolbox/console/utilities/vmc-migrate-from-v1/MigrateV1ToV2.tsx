    ...
    <>
      <div className="space-y-6">
        <div className="bg-zinc-100 dark:bg-zinc-800 p-4 rounded-xl text-sm mb-4">
          <p className="mb-2">
            <strong>Note:</strong> This tool is only required if your L1 has the Validator Manager contract version 1
            deployed. If you have deployed the Validator Manager contract with this Toolbox, it is already the version
            2. In this case you don't need to do this!
          </p>
        </div>
        <div>
          <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-4">
            This tool allows you to migrate a validator from the V1 contract to the V2 contract. Before using this tool,
            ...
            You need to provide the validation ID, the latest nonce received from the P-Chain, and the address of the
        ...
        <form className="space-y-4">
          ...
          <Button onClick={handleMigrate} loading={isProcessing} loadingText="Migrating..." disabled={isProcessing}>
            Migrate Validator
          </Button>

          {error && <div className="text-red-500 text-sm p-3 bg-red-50 dark:bg-red-900/30 rounded-xl">{error}</div>}
