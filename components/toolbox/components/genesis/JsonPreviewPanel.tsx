...
export function JsonPreviewPanel({
  ...
  const findLineNumberForPath = (
    lines: string[],
    path: string,
    pathMap: Record<string, { section: string; offset: number }>,
  ): number | null => {
    try {
            ...
            <div className="text-[11px] leading-5">
              {jsonLines.length > 0 && highlightedLine ? (
                <div className="relative">
                  <SyntaxHighlightedJSON code={jsonData} highlightedLine={highlightedLine} />
                </div>
              ) : (
                <SyntaxHighlightedJSON code={jsonData} highlightedLine={null} />
              )}
            </div>
          ) : (
